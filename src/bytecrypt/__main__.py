...
def init_argparse() -> ArgumentParser:
    ...
    mode.add_argument(
        "-e", "--encrypt", action="store_true", help="encrypt the target"
    )
    mode.add_argument(
        "-d", "--decrypt", action="store_true", help="decrypt the target"
    )
    mode.add_argument(
        "--reencrypt",
        action="store_true",
    ...
    target.add_argument(
        "-dir", "--directory", help="use a directory as the target"
    )
    ...
    parser.add_argument(
        "-np", "--new-password", help="the new password, with --reencrypt"
    )
    parser.add_argument(
        "-r",
        "--recursive",
...
def run(args) -> None:
    ...
    if args.encrypt:
        ...
        if args.directory:
            encrypt_directory(
                args.directory, password, name_action, args.recursive,
                args.force
            )
        elif args.file:
            encrypt_file(
        ...
        password = resolve_password(args, confirm=False)
        if args.directory:
            count = reencrypt_directory(
                args.directory, password, args.new_password, args.recursive
            )
            print_info("re-encrypted %d file(s)" % count)
        else:
            changed = reencrypt_file(args.file, password, args.new_password)
            print_info(
                "re-encrypted" if changed else "already at the current "
